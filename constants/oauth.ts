...
export function getApiBaseUrl(): string {
  ...
  if (API_BASE_URL) {
    ...
    console.log("🌐 [API URL] Using configured API_BASE_URL:", url);
    return url;
  }
  ...
  if (ReactNative.Platform.OS === "web" && typeof window !== "undefined" && window.location) {
    ...
    if (apiHostname !== hostname) {
      ...
      console.log("🌐 [API URL] Derived from web hostname:", url);
      return url;
    }

    // Fallback for web: use localhost with port 3000
    const url = `${protocol}//localhost:3000`;
    console.log("🌐 [API URL] Web fallback to localhost:", url);
    return url;
  }

  // Mobile: warn and use empty (will likely fail)
  console.warn("⚠️ [API URL] No API_BASE_URL configured for mobile!");
  console.warn("⚠️ [API URL] Please set EXPO_PUBLIC_API_BASE_URL in .env to your computer's IP");
  console.warn("⚠️ [API URL] Example: EXPO_PUBLIC_API_BASE_URL=http://192.168.1.100:3000");
  return "";
}
